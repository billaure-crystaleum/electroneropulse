...
electronero network - GitHub: [github.com/electronero/electronero](https://github.com/electronero-pulse/electroneropulse)
...
To run in background:
    ...
    ./bin/pulsed --log-file pulsed.log

To run as a systemd service, copy
[pulsed.service](utils/systemd/pulsed.service) to `/etc/systemd/system/` and
[pulsed.conf](utils/conf/pulsed.conf) to `/etc/`. The [example
service](utils/systemd/pulsed.service) assumes that the user `electronero` exists
and its home is the data directory specified in the [example
config](utils/conf/pulsed.conf).

If you're on Mac, you may need to add the `--max-concurrency 1` option to
electronero-wallet-cli, and possibly pulsed, if you get crashes refreshing.
